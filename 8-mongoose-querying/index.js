...
app.get("/students/", async (req, res, next) => {
  ...
  let students = await Student.find()
    .sort("-firstname") // ascending / descending
    // .limit(2) // limits the amount of documents returned to the amount specified 
    // .skip(1) // ignore the first document (=student)

  res.send(students)
})


// LET'S TRAIN POPULATING REFERENCES
app.get("/courses", async (req, res, next) => {

  // give me the list of all courses
  let courses = await Course.find() // QUERY
    .select("-_id room.floor room.nr title")
    .populate("students", "firstname -_id") // lookup of referenced document & dynamic nesting of it
    .populate("teachers")
    .where("room.floor").equals(2)
    //.where("title").equals("FBW23") // filter data by certain criteria
...
// OUR SEED ROUTE
// (it is "async" - so we can use await in there :)
